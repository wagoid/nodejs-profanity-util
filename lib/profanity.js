...
function getListRegexAsync (list, languages) {
  if (list) {
    ...
    return Promise.all(getWordListsPromises(languages))
    .then(function (listsArray) {
      ...
      return _getListRegex(listRegex);
    });
  }
}
...
function checkAsync (target, languages, forbiddenList) {
  ...
  return getListRegexAsync(forbiddenList, languages)
    .then(function (regex) {
...
function getReplacementsList (options) {
  var replacementsList = options.replacementsList;
  if (!_.isArray(replacementsList) || _.isEmpty(replacementsList)) {
...
function purify (target, options) {
  ...
  options.replacementsList = getReplacementsList(options);
...
function purifyAsync (target, options) {
  ...
  var fields = options.fields || (target instanceof Object ? Object.keys(target) : []);
  return getListRegexAsync(options.forbiddenList, options.languages)
    .then(function (regex) {
      return _purify(target, fields, regex, options);
    });
}

...
function promisiFyReadFile(fileName, option) {
...
function getWordListsPromises (languages) {
  ...
  _.each(languages, function(language) {
    promises.push(promisiFyReadFile('./swearwords/' + language, DEFAULT_FS_OPTIONS));
  });
...
function getWordListsConcatenated (languages) {
  ...
  _.each(languages, function (language) {
    lists.concat(JSON.parse(fs.readFileSync('./swearwords/' + DEFAULT_FS_OPTIONS)));
  });
...
function getListRegexAsync (list, languages) {
  var listRegex = [];
  if (list) {
    listRegex = new Promise(function (resolve, reject) { resolve(_getListRegex(list)); } );
  } else {
    Promise.all(getWordListsPromises(languages))
      .then(function (listsArray) {
        _.each(listsArray, function (lst) {
          listRegex.concat(_getListRegex(lst));
        });
        resolve(listRegex);
      })
      .catch(function (err) {
...
function check (target, forbiddenList, languages) {
  languages = getDefaultLanguagesValue(languages);

  var regex = forbiddenList ? getListRegex(forbiddenList) : getListRegex(null, languages);
...
function checkAsync (target, forbiddenList, languages) {
  languages = getDefaultLanguagesValue(languages);
...
function purify (target, options) {
  ...
  var matches = [],
    ...
    result, regex = forbiddenList ? getListRegex(forbiddenList) : getListRegex(null, languages);

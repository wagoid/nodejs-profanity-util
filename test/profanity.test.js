...
function callTestCheckFunctions(languages, forbiddenList) {
  it('Should return null with no swearwords found in string', () => {
    should(profanity.check('No swearwords here', languages, forbiddenList)).eql([]);
  });

  it('Should return array of swearwords found in dirty string', () => {
  ...
  it('Should not target substrings', () => {
  ...
  it('Should work equally for objects (Recursively) and arrays', () => {
...
describe('Profanity module', () => {
  ...
  describe('.check(target) when languages="all"', () => {
    callTestCheckFunctions('all', null);
  });

  describe('.check(target) for portuguese', () => {
    it('Should not get a portuguese dirty word when there is no language specifided (default is english)', () => {
      var notDetected = profanity.check('viados não gostam de pepeca', null);

      should(notDetected).have.length(0);
    });

    it('Should get a dirty word and not get substrings', () => {
  ...
  describe('.check(target) for multiple languages', () => {
    it('Should get a dirty word and not get substrings', () => {
  ...
  describe('.checkAsync(target)', () => {
    it('Should return null with no swearwords found in string', () => {
      return profanity.checkAsync('No swearwords here')
        .then(results => {
          should(results).eql([]);
        });
    });
    ...
    it('Should return array of swearwords found in dirty string', () => {
      return profanity.checkAsync('something damn something something poo something')
        .then(results => {
          should(results).eql([
            'damn',
            'poo'
    ...
    it('Should work when passing my own forbidden list', () => {
      return profanity.checkAsync('something damn something something poo something', 'en', ['damn', 'poo'])
        .then(results => {
          should(results).eql([
            'damn',
            'poo'
  ...
  describe('.purify(target)', () => {
    it('Should work in obscure (default) mode on a simple string', () => {
    ...
    it('Should work in obscure (default) mode recursively with objects, with infinite recursion and maxRecursionDepth', () => {
      var testObj = {
    ...
    it('Should work in replace mode on a simple string', () => {
      var result = profanity.purify('boob damn something poo', {
        replace: true
      });
    ...
    it('Should work in replace mode recursively with objects', () => {
  ...
  describe('.purifyAsync(target)', () => {
    it('Should work in obscure (default) mode on a simple string', () => {
      return profanity.purifyAsync('boob damn something poo')
        .then(result => {
          result[0].should.equal('b**b d**n something p*o');
          result[1].should.eql(['boob', 'damn', 'poo']);
        });
    ...
    it('Should work in obscure (default) mode recursively with objects', () => {
      var objToCheck = {
        bar: { foo: 'something boob', bar: { foo: 'test poo' } },
        test: 'something damn'
      };
      return profanity.purifyAsync(objToCheck)
        .then(result => {
          result[0].should.have.keys('bar', 'test');
          result[0].bar.should.have.keys('foo', 'bar');
          result[0].bar.foo.should.equal('something b**b');
    ...
    it('Should work in replace mode on a simple string', () => {
      return profanity.purifyAsync('boob damn something poo', { replace: true })
        .then(result => {
          util.testPurified(result[0], '[ placeholder ] [ placeholder ] something [ placeholder ]');
          result[1].should.eql(['boob', 'damn', 'poo']);
        });
    ...
    it('Should work in replace mode recursively with objects', () => {
      ...
      return profanity.purifyAsync(objToCheck, { replace: true })
        .then(result => {
          result[0].should.have.keys('bar', 'test');
          result[0].bar.should.have.keys('foo', 'bar');
          util.testPurified(result[0].bar.foo, 'something [ placeholder ]');
